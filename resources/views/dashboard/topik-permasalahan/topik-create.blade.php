        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Tambah Topik Permasalahan</h3>
                </div>
                <form action="{{ route('topik-permasalahan.store') }}" method="POST">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="aqr_option_id"
                                ...
                                @foreach ($options as $opt)
                                    <option value="{{ $opt->id }}" {{ old('aqr_option_id') == $opt->id ? 'selected' : '' }}>
                                        {{ $opt->nama_option }}
                                    </option>
                                @endforeach
                            </select>
