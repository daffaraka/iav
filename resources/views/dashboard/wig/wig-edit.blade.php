                            ...
                            <form action="{{ route('wig.update', $wig->id) }}" method="POST">
                                ...
                                <div class="form-group my-3">
                                    ...
                                    <input required type="text" name="nama_wig" id=""
                                        value="{{ $wig->judul_wig ?? old('judul_wig') }}"
                                ...
                                <div class="form-group my-3">
                                    <div class="row">
                                        <div class="col-6">
                                            ...
                                            <input type="number" name="from_x" class="form-control"
                                                value="{{ $wig->from_x ?? old('from_x') }}">
                                        </div>
                                ...
                                {{-- <div class="form-group my-3">
                                    ...
                                    <select required name="unit_id"
                                        ...
                                        <option value="IT Branding"
                                            {{ ($wig->unit_id ?? old('unit_id')) == 'IT Branding' ? 'selected' : '' }}>IT &
                                            Branding</option>
                                        <option value="SDM"
                                            {{ ($wig->unit_id ?? old('unit_id')) == 'SDM' ? 'selected' : '' }}>SDM</option>
                                        <option value="RND"
                                            {{ ($wig->unit_id ?? old('unit_id')) == 'RND' ? 'selected' : '' }}>RND</option>
                                ...
                                <button class="btn btn-primary mt-3 shadow" type="submit" id="updateWig">Submit</button>
                            </form>
...
@push('scripts')
    <script>
        // $('#updateWig').click(function(e) {
        //     e.preventDefault();

        //     // $.ajax({
        //     //     url: "{{ route('wig.update', ['wig' => $wig->id]) }}",
        //     //     type: "PUT",
