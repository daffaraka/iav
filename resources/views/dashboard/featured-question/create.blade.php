            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">{{ $title ?? 'Tambah Featured Question' }}</h3>
                    </div>
                    <form action="{{ route('featured-question.store') }}" method="POST">
                        @csrf
                        <div class="card-body">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <textarea class="form-control @error('jawaban') is-invalid @enderror" id="jawaban" name="jawaban" rows="6"
                                    required placeholder="Masukkan jawaban">{{ old('jawaban') }}</textarea>
                                @error('jawaban')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
