...
export default function MasterKelasIndex({ kelas }) {
    ...
    const columns = useMemo(() => [
        ...
        {
            accessorKey: 'wali_kelas.name',
            header: 'Wali Kelas',
            cell: info => {
                const wali = info.getValue();
                return wali ? (
                    <div className="inline-flex items-center px-3 py-1.5 rounded-lg bg-surface-100 dark:bg-surface-700/50 text-slate-800 dark:text-slate-200 text-sm font-semibold tracking-wide border border-surface-200 dark:border-surface-600">
                        <i className="ph ph-chalkboard-teacher text-brand-500 mr-2 text-lg"></i>
                        {wali}
                    </div>
                ) : (
        ...
        },
        {
            ...
            header: 'Aksi',
            cell: info => {
                const kls = info.row.original;
                return (
                    <div className="flex items-center gap-2">
                        <Link 
                            href={`/master-kelas/${kls.id}/edit`}
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Manajemen Master Kelas" />

            <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Master Kelas</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Kelola daftar kelas dan wali kelas</p>
                </div>
                <Link 
                    href="/master-kelas/create" 
