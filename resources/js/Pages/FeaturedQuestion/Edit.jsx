...
export default function Edit({ fq, title }) {
    const { data, setData, put, processing, errors } = useForm({
        judul: fq.judul || '',
        jawaban: fq.jawaban || '',
        kategori: fq.kategori || '',
        is_pinned: fq.is_pinned === 1 || fq.is_pinned === true,
        is_published: fq.is_published === 1 || fq.is_published === true,
        order: fq.order || 0,
        tiket_id: fq.tiket_id || ''
    });

    const submit = (e) => {
        e.preventDefault();
        put(`/featured-question/${fq.id}`);
    };

    return (
        <AuthenticatedLayout>
            <Head title={title || "Edit Featured Question"} />

            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-white">{title || "Edit Featured Question"}</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Ubah detail FAQ</p>
                </div>
                <Link 
