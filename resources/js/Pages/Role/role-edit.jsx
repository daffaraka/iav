...
export default function RoleEdit({ title, role, permissions, rolePermissions }) {
    // rolePermissions in RoleController is passed as an object: {'id': 'id', ...} 
    // We need to map it to names or just keep checking IDs if rolePermissions comes as array of IDs.
    // In RoleController.php it was:
    // pluck('role_has_permissions.permission_id','role_has_permissions.permission_id')->all()
    // This gives an object like: { '1': '1', '4': '4' }
    
    // We want to work with permission names in data.permission to match what store/update expects.
    // So we need to map those IDs back to names using the `permissions` array.
    const initialPermissionNames = [];
    const rolePermissionIds = Object.values(rolePermissions);
    
    permissions.forEach(p => {
        if (rolePermissionIds.includes(p.id) || rolePermissionIds.includes(p.id.toString())) {
            initialPermissionNames.push(p.name);
        }
    });

    const { data, setData, put, processing, errors } = useForm({
        name: role.name || '',
        permission: initialPermissionNames
    });
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-white">{title || "Edit Role"}</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Ubah detail role: <span className="font-bold text-brand-500">{role.name}</span></p>
                </div>
                <Link 
                    href="/role" 
