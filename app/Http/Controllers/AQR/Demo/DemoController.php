...
namespace App\Http\Controllers\AQR\Demo;
...
class DemoController extends Controller
{
    public function create(Request $request)
    {
        ...
        if ($request->get('pengirim') == null) {
            return redirect()->route('helpdesk.demo.cek-pengirim');
        }
        $pengirim = $request->get('pengirim');

        if ($pengirim === 'Masyarakat Umum') {
            return view('frontend.aqr.lovable.open-tiket-umum-new', compact('pengirim'));
        } else {
            $options = AqrOption::orderBy('nama_option', 'asc')->get();

            return view('frontend.aqr.lovable.open-tiket-warga-new', compact(['pengirim', 'options']));
        }
    }

    public function cekPengirim()
    {
        return view('frontend.aqr.lovable.cek-pengirim-new');
    }
    ...
    public function storeTiket(Request $request)
    {
        ...
        if ($request->pengirim === 'Warga Sekolah' && $request->nisn) {
            $siswa = MasterSiswa::where('nisn', $request->nisn)->first();
    ...
    {
        return view('frontend.aqr.lovable.layout-new');
    }
    ...
    public function show($tiket)
    {
        $tiket = Tiket::with('first_pic', 'pic', 'progres', 'option')
            ...
            ->first();
        // dd($tiket);
        return view('frontend.aqr.lovable.detail-tracking-tiket-new', compact('tiket'));
    }
