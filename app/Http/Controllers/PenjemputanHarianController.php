...
// use App\Events\TestNotification; // Ensure this exists or comment out
use App\Models\PenjemputanHarian;
use Illuminate\Support\Facades\Auth;
// use App\Jobs\SendPenjemputanNotification; // Ensure this exists or comment out
...
class PenjemputanHarianController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Auth::user()->hasRole(['super-admin', 'satpam', 'kepala-sekolah', 'humas'])) {
            ...
            // For Guru / Wali Kelas
            // Assuming pic_kelas in user matches kelas in MasterSiswa, or they only see their assigned class
            $pic_kelas = Auth::user()->pic_kelas ?? '1A'; // Default or adjust based on logic
            $siswaDijemput = PenjemputanHarian::whereDate('created_at', Carbon::now()->format('Y-m-d'))
                ->with(['siswa' => function ($query) use ($pic_kelas) {
                    $query->where('kelas', $pic_kelas);
                }])
                ->get()
                ->groupBy('siswa.kelas')
                ->sortKeys();
            ...
            $penjemputan = PenjemputanHarian::whereDate('created_at', Carbon::now()->format('Y-m-d'))
                ->with('siswa')
                ->whereHas('siswa', function ($query) use ($pic_kelas) {
                    $query->where('kelas', $pic_kelas);
                })
                ->orderBy('waktu_dijemput', 'desc')
                ->get();
    ...
    {
        $pic_kelas = Auth::user()->pic_kelas ?? $kelas;
        $siswaDijemput = PenjemputanHarian::whereDate('created_at', Carbon::now()->format('Y-m-d'))
            ->with('siswa')
            ->whereHas('siswa', function ($query) use ($pic_kelas) {
                $query->where('kelas', $pic_kelas);
    ...
    public function penjemputDatang(Request $request)
    {
        ...
        if ($penjemputan) {
            ...
            // dispatch(new SendPenjemputanNotification($penjemputan->siswa)); // Uncomment when Jobs are migrated
    ...
    {
        try {
            if (!Auth::user()->hasRole(['super-admin', 'satpam'])) {
                $pic_kelas = Auth::user()->pic_kelas ?? '1A';
                $siswa_kelas = MasterSiswa::where('kelas', $pic_kelas)->get();
                ...
                for ($i = 0; $i < count($siswa_kelas); $i++) {
                    PenjemputanHarian::create([
                        'pic_id' => Auth::user()->id,
                        'master_siswa_id' => $siswa_kelas[$i]->id,
                    ]);
                }
            }
            ...
            return redirect()->route('penjemputan-harian.index')
                ->with('error', 'Gagal menambahkan penjemputan. Silakan coba lagi. ' . $e->getMessage());
        }
    }
    ...
    public function satpamKonfirmasiOjol(Request $request)
    {
        ...
        if ($request->has('ojol') && $penjemputan) {
            $penjemputan->update([
                'waktu_dijemput' => Carbon::now(),
        ...
        return redirect()->route('penjemputan-harian.index')
            ->with('success', 'Satpam telah mengkonfirmasi Ojol.');
    }
    ...
    public function nullPenjemputan()
    {
        try {
            ...
            return redirect()->back()->with('success', 'Penjemputan berhasil di-reset.');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Penjemputan gagal di-reset.');
        }
    }
}
